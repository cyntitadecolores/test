   ...
   app.get('/proyecto/:id_socio/postulados', (req, res) => {
    ...
    const query = `
      SELECT
        P.id_proyecto,
        P.nombre_proyecto,
        P.cupos_proyecto AS cupo_max, -- Añadido
        (
          SELECT COUNT(*)
          FROM Postulacion
          WHERE id_proyecto = P.id_proyecto
            AND status IN ('Aceptadx', 'Inscrito')
        ) AS aceptados, -- Añadido
        JSON_ARRAYAGG(
          JSON_OBJECT(
      ...
      GROUP BY P.id_proyecto, P.nombre_proyecto, P.cupos_proyecto; -- Añadido P.cupos_proyecto a GROUP BY
    `;
...
app.put('/postulacion/:id_proyecto/:id_estudiante', (req, res) => {
  ...
  /* --------------------------------------------
     1.  Si va a aceptar, verifica cupo disponible
  -------------------------------------------- */
  if (status === 'Aceptadx') {
    const qry = `
      SELECT
        P.cupos_proyecto                         AS cupo_max,
        COUNT(Po.id_postulacion)                 AS aceptados
      FROM   Proyecto    P
      LEFT JOIN Postulacion Po
        ON Po.id_proyecto = P.id_proyecto
       AND Po.status IN ('Aceptadx','Inscrito')
      WHERE  P.id_proyecto = ?
      GROUP BY P.cupos_proyecto`;

    db.query(qry, [id_proyecto], (err, [row]) => {
      if (err) {
        console.error('❌ Error al validar cupo:', err);
        return res.status(500).json({ message: 'Error del servidor' });
      }
      if (!row) return res.status(404).json({ message: 'Proyecto no encontrado' });

      if (row.aceptados >= row.cupo_max) {
        return res
          .status(409)                       // 409 Conflict
          .json({ message: 'Cupo de estudiantes lleno para este proyecto' }); // Este es el mensaje clave
      }

      // hay espacio → continuar con el UPDATE
      terminarActualizacion();
    });
    return;               // evita que caiga al UPDATE sin revisar
  }

  terminarActualizacion(); // casos que no son “Aceptadx”
  function terminarActualizacion() {
   const query = `
     UPDATE Postulacion
        SET status = ?
      WHERE id_proyecto = ? AND id_estudiante = ?
   `;
   db.query(query, [status, id_proyecto, id_estudiante], (err, _r) => {
     if (err) {
